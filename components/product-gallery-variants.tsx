...
export default function ProductGalleryVariants({ product, variants }: Props) {
  ...
  useEffect(() => {
    const detail = { productId: product.id, variantId: selected.id, color: (selected as Product).color, connectionType: (selected as Product).connectionType, size: (selected as Product).size }
